<?php 
	...
	if (isset($_POST['submit'])) {
		...
		if (empty($userid) || empty($first_name) || empty($last_name) || empty($dob) || empty($gender) || empty($email) || empty($phone) || empty($apply_for)) {
			...
			try {
				$stmt = 
				$stmt = $mysqli->prepare("INSERT INTO users (type, first_name, last_name, dob, gender, email, phone, phone2, apply_for, company_register, userid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				$stmt->bind_param("sssssssssis", $type, $first_name, $last_name, $dob, $gender, $email, $phone, $phone2, $apply_for, $company_register, $userid);
				if ($stmt->execute()) {
					...
					echo $userid, $sector, $category, $idea, $solution_to, $your_solution, $competitors, $last_funding, $revenue, $kuberan_house, $share;
