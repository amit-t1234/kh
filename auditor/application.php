<?php 
  ...
  if (isset($_POST['submit'])) {
    ...
    $stmt->bind_param('is', $answer, $_POST['userid']);
    $stmt->execute();
        ...
        <div class="container pb-3">
          <form action="" method="POST">
            <div class="row mt-3" style="color:black;font-weight:bold;">
                ...
                <?php
                  $stmt = $mysqli->prepare("SELECT * FROM Users u JOIN Extras e ON u.userid = e.userid AND u.userid=? JOIN company c ON u.userid = c.userid JOIN Profiles p ON c.userid = p.userid JOIN Attachments a ON a.userid = u.userid JOIN Business b ON b.userid = u.userid ORDER BY created_on ASC");
                ...
                <div class="row mt-3">
                 ...
                 <div class="col-md-4"><?php echo $row["userid"] ?></div>
               </div>
                <div class="row mt-3">
    ...
    <!--             <div class="row mt-3">
               ...
               <?php 
                if ($row['status'] == '3') {
                  ...
                  echo '<button name="submit" value="4" class="btn btn-lg btn-danger">Reject</button>';                  
                } else if ($row['status'] == '4') {
                  echo '<p class="text-danger">Rejected</p>';
                  echo '<button name="submit" value="3" class="btn btn-lg btn-success">Accept</button>';
                } else {
              ?>
               <button name="submit" value="3" class="btn btn-lg btn-success">Accept</button>
               <button name="submit" value="4" class="btn btn-lg btn-danger">Reject</button>
             <?php } ?>
               </div>            
          </form>
        <!-- Begin Page Content -->
